...
const GameCanvas = forwardRef<GameCanvasHandle, GameCanvasProps>(({ assets }, ref) => {
  ...
  const draw = useCallback((game: Game, hands: HandData[], now: number) => {
    ...
    if (ball && (ball.active || game.state === STATE.RESULT)) {
      const bx = golX + ball.x * golW;
      const by = golY + ball.y * golH;
      const br = ball.radius * Math.min(w, h);

      // Arrow/target indicator
      if (game.shouldShowArrow() && ball.progress < 0.5) {
      ...
      if (assets.bola) {
        ...
        ctx.drawImage(assets.bola, -size / 2, -size / 2, size, size);
        ctx.restore();
      }
    }
